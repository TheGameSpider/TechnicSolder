...
if($_GET['type']=="update") {
	mysqli_query($conn, "INSERT INTO builds(`name`,`minecraft`,`java`,`mods`,`modpack`) SELECT `name`,`minecraft`,`java`,`mods`,`modpack` FROM `builds` WHERE `modpack` = '".$_GET['id']."' ORDER BY `id` LIMIT 1");
