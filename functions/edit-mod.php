...
if($mod['pretty_name']!==$_POST['pretty_name']){
	mysqli_query($conn, "UPDATE `mods` SET `pretty_name` = '".$_POST['pretty_name']."' WHERE `id` = ".$_GET['id']);
}
if($mod['link']!==$_POST['link']){
	mysqli_query($conn, "UPDATE `mods` SET `link` = '".$_POST['link']."' WHERE `id` = ".$_GET['id']);
}
if($mod['author']!==$_POST['author']){
	mysqli_query($conn, "UPDATE `mods` SET `author` = '".$_POST['author']."' WHERE `id` = ".$_GET['id']);
}
...
if($mod['description']!==$_POST['description']){
	mysqli_query($conn, "UPDATE `mods` SET `description` = '".$_POST['description']."' WHERE `id` = ".$_GET['id']);
}
